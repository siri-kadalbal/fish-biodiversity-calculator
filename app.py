...
# ══════════════════════════════════════════════════════
# STARTUP (LEAN VERSION)
# ══════════════════════════════════════════════════════
BUNDLE_PATH = "deployable_bundle.joblib"

print("Loading pre-calculated model bundle...")
data_bundle = joblib.load(BUNDLE_PATH)

# Setup Full Model
FEATS_F   = data_bundle["full"]["features"]
SCALER_F  = data_bundle["full"]["scaler"]
IMPUTER_F = data_bundle["full"]["imputer"]
MODEL_F   = data_bundle["full"]["model"]
# We mock X_ORIG_F by providing the pre-calculated medians and quantiles
class MockDF:
    ...
    def __getitem__(self, key):
        # This handles the X_orig[feat].quantile() calls
        class Col:
            def __init__(self, q): self.q = q
            def quantile(self, val): return self.q[0] if val < 0.5 else self.q[1]
            def median(self): return medians[key]
        return Col(self.quantiles[key])

X_ORIG_F = MockDF(data_bundle["full"]["medians"], data_bundle["full"]["quantiles"])

# Setup Human Model
FEATS_H   = data_bundle["human"]["features"]
SCALER_H  = data_bundle["human"]["scaler"]
IMPUTER_H = data_bundle["human"]["imputer"]
MODEL_H   = data_bundle["human"]["model"]
X_ORIG_H = MockDF(data_bundle["human"]["medians"], data_bundle["human"]["quantiles"])
